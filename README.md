...
- `API 配置` 里的 `系统提示词` 就是 **normal（正常聊天）模式** 的提示词。
- `编程模式配置` 里的 `编程模式系统提示词` 对应 **coding（编程）模式**。
- `React` 模式是预留模式，对应 `modePrompts.react`，当前主要用于后续扩展。
- 后端会根据当前聊天模式选择对应提示词，并把当前代码上下文、记忆、草稿/固化代码一并发给模型。
- 也就是说：普通聊天看 `systemPrompt`，编程模式看 `modePrompts.coding`，两者是分开的。
...
- `运行限制开关` 打开时，下面的各项限制才生效。
- 关闭时，**只是不再启用这些限制**，不会把原来的数值都写成 `0`。
- 也就是说，Flash 里会保留你上次填写的限制值；只是运行时按“无限制”执行。
- 重新打开开关后，会继续使用之前保存的数值，而不是被清空的值。
...
- `运行日志上限`（字符，默认 `16000`）：`run.log` 的最大保留字符数，超出会保留末尾内容，`0` 表示不限制。
...
- `输出 / 错误` 会显示本次运行的最后结果，数据来自内存中的运行态。
- `运行日志` 会写入 `data/code/run.log`，用于记录运行、停止、保存配置等操作。
- 两个区域右侧都有一个小的 `清空` 按钮：
  - 清空 `输出 / 错误`：只清空当前窗口显示。
  - 清空 `运行日志`：会把 `run.log` 文件清空。
- 这两个清空操作不会影响当前代码草稿、固化版本或历史记录。
